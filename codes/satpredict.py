...
def show_filter_responses(model, layer_index, input_img_path, save_dir=None, filter_index=None, dpi=100.0, save_original=False):
    ...
    display_image = utility.deprocess_image(display_image, alter_dim=False)

    pl.figure(
        figsize=(display_image.shape[0] / dpi, display_image.shape[1] / dpi),
        dpi=dpi
    )
    utility.nice_imshow(pl.gca(), display_image, cmap=cm.binary)
    ...
    if filter_index is None:
        ...
        out_img = utility.make_mosaic(c1, grid_dim, grid_dim)
    else:
    ...
    if save_dir is not None:
        ...
        misc.imsave(os.path.join(save_dir, prefix + "_output.png"), utility.deprocess_image(out_img, alter_dim=False))
    ...
    utility.nice_imshow(pl.gca(), out_img, cmap=cm.binary)
